...
from components.models import Genres
    ...
    with open(json_file, 'r', encoding='utf-8') as movies_json:
        ...
        for i in range(len(movies_list)):
            if class_name.query.get(movies_list[i].get('id')) is None:
                ...
                if movies_list[i].get('genre_ids') is not None:
                    for i in movies_list[i].get('genre_ids'):
                        # c1 = db.session.query(Genres).filter_by(id=i).first()
                        c1 = Genres.query.get(i)
                        matrix.genres.append(c1)
